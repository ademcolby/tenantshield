...
**Arizona** — ARS §33-1321(D). Return within 14 BUSINESS days (excluding Saturdays, Sundays, and legal holidays — NOT calendar days) after termination of tenancy AND delivery of possession AND written demand by tenant. For the exact deadline date, use the "14 business days after move-out" line from the BUSINESS-DAY deadline candidates in the PRE-CALCULATED DATES block (it already excludes weekends and federal holidays) — do NOT use a calendar-day line and do NOT compute it yourself. Wrongful withholding: tenant may recover the amount wrongfully withheld PLUS damages up to 2x the amount wrongfully withheld. Note: the 14-business-day clock requires all three triggers (termination + possession + tenant demand). Deposit cap: 1.5 months' rent (§33-1321(A)) — raise it where an excess deposit was charged. If the tenant does not dispute an itemized list within 60 days of it being mailed, the amounts are deemed final — the demand letter serves as that timely dispute.
...
**Connecticut** — CGS § 47a-21. CONDITIONAL DEADLINE (whichever is LATER): the landlord must return the deposit with interest, or provide a written itemized statement of damages, within (a) 21 days after termination of the tenancy, OR (b) 15 days after receiving the tenant's forwarding address, whichever is later. The 21-day deadline took effect October 1, 2023 (reduced from 30 days) — many secondary sources still cite the stale 30-day figure; always use 21. CALCULATION: If the input includes a forwarding-address date (forwardingAddressDate field), compute both candidate deadlines and use the later one in the letter. If no forwarding-address date is provided, rely on the 21-day-from-termination deadline and note that the 15-day-after-forwarding-address alternative may extend it. Wrongful withholding: TWICE THE SECURITY DEPOSIT for violating the return/statement provisions under § 47a-21(d), plus accrued interest. (If the ONLY violation is failure to deliver accrued interest, the remedy is $10 or twice the interest, whichever is greater.)
