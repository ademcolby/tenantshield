...
// July 6, 2026 update (Phase 5 — Project K Phase 1 audit flags): Colorado deposit cap corrected to TWO
// months' rent per SB23-184 (2023) — HB25-1249 did NOT create a 1-month cap (prior text was inverted);
// Connecticut deadline corrected 30→21 days (eff. 10/1/2023) + penalty base aligned to 2x the security
// deposit per § 47a-21(d); Kentucky "30/60-day return deadline" misreading removed — § 383.580's real
// structure is escrow/disclosure/listing duties with a forfeiture remedy (30-day rule = unpaid-rent
// application, 60-day rule = tenant-response window); Tennessee penalty reframed as forfeiture + full
// deposit + court costs (no multiplier) + § 66-28-301(f) 60-day response trap added.
...
**Arizona** — ARS §33-1321(D). Return within 14 BUSINESS days (excluding Saturdays, Sundays, and legal holidays — NOT calendar days) after termination of tenancy AND delivery of possession AND written demand by tenant. For the exact deadline date, use the "14 business days after move-out" line from the BUSINESS-DAY deadline candidates in the PRE-CALCULATED DATES block (it already excludes weekends and federal holidays) — do NOT use a calendar-day line and do NOT compute it yourself. Wrongful withholding: tenant may recover the amount wrongfully withheld PLUS damages up to 2x the amount wrongfully withheld. Note: the 14-business-day clock requires all three triggers (termination + possession + tenant demand).
