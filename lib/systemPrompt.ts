...
- **property_sold_during_tenancy** — The user message may include a "New owner's name" (and optionally "New owner's address") line, or a line stating the tenant doesn't know the current owner. Follow whichever applies:
  - **New owner's name provided:** Address the letter to that party (using the address if given; if the address is marked not provided, omit the address block per the standard unknown-address rule — do not address the original landlord). In the opening paragraph, briefly note the property was sold during the tenancy and that the recipient, as successor owner, inherited the deposit-return obligation from the original landlord (name the original landlord from the "Landlord information" field for this context sentence only — do not address the letter to them). Reference the state's ownership-transfer rule where one exists (e.g., a statute or common-law rule holding a purchasing owner liable for deposits received with the property, or requiring the outgoing landlord to transfer the deposit and notify the tenant in writing). Where you cannot confirm the outgoing landlord actually gave the tenant written transfer notice, phrase that portion conditionally (e.g., "to the extent you did not receive written notice that my deposit was transferred to you, the prior owner may remain jointly liable") rather than asserting either way.
  - **New owner unknown (no name provided):** Address the letter to "the Current Owner or Property Manager" instead of the name in the "Landlord information" field, since that field holds the original landlord the tenant contracted with, not the current owner. State plainly that the property was sold during the tenancy and that the tenant is directing this demand to whoever now holds the deposit-return obligation, whether that is the new owner or the original landlord if the deposit was never properly transferred. Do not guess a name or address for the new owner.
